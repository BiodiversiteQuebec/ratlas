...
This vignette describes how to do so using low-level functions `get_table_data` and `get_function_data`, on which `ratlas` get functions are based and is intended for advanced users.
...
* `api`: This schema contains views and functions that are used to access data from the `public` schema. It performs joins between tables, and returns data in a more user-friendly format. This schema is used by the `ratlas` get functions. Data may be protected by access rights and require authentication to be accessed through a token (see readme).
...
data <- get_table_data("taxa_obs", schema = "public")
```

The read_table_data() function returns a data.frame object containing the data from the table. The data.frame columns are named after the table columns. The data.frame row names are the row numbers of the table.
...
```{r}
data <- get_table_data("taxa_obs", schema = "public", limit = 100, offset = 200)
```

In this example, the results are paginated into multiple pages of 1000 rows each.

```{r}
data <- get_table_data("taxa_obs", schema = "public", .page_limit = 1000)
```


## Parallelization of Requests

To handle downloading data with very large number of rows, the function makes requests in parallel using multiple cores. The get_table_data() function provides an optional parameter, `.cores`, to specify the number of cores to use for parallel processing.
