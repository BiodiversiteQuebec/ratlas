...
vignette: >
  %\VignetteIndexEntry{Download observations by region}
...
The database `obs_regions_taxa_datasets_counts` view contains the number of observations for each region, taxa and datasets. We can query it directly with the `get_gen()` function.

```{r}	
sherbrooke_count_response <- get_gen(
  "obs_regions_taxa_datasets_counts",
  .schema = "atlas_api",
  fid = sherbrooke$fid
  # id_taxa_obs = 735,
  )
...
```{r}
sherbrooke_observations <- get_observations(fid_region = sherbrooke$fid, year = 2002)
