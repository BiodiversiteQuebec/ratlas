...
#' species specified by `id_taxa_ref`.
#'
#' The absence are infered from all sampling points related to birds observed
#' within Quebec terrestrial limits whose observations were obtained from
#' datasets defined as `exhaustive`.
#'
...
#' pres_abs <- get_bird_presence_absence(rank = "species")
#'
#' @export

list_bird_taxa <- function(
    rank = NULL,
    source_name = NULL,
    valid = NULL,
    ...
  ...
  query <- list(...)
  query$endpoint <- "bird_quebec_taxa_ref"
  query$.schema <- "api"
